...
% using uncorrelated updating exploration rate.
...
for iter=1:n_iter
    ...
    for i=1:min(iter,10)
        ...
        temp_W = (basis.^2./(sum(basis.^2.*(ones(length(rt),1)*variance(:,j)'),2)*ones(1,n_rfs)))';
    ...
    if iter>1
        ...
        variance(:,iter+1) = max(var_nom./(var_dnom+1.e-10), 0.1.*variance(:,1));
    ...
    if iter~=n_iter
        noisE = variance(:,iter+1).^.5.*randn(n_rfs,1);
        %disp(noisE)
...
save('uncor_learned.txt','-ascii','Return')
