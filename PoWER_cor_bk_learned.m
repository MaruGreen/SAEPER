...
% using correlated updating exploration rate.
...
rate = zeros(n_rfs,n_iter+1);
basis = zeros(n_rfs,length(rt));
randn('state',20);

% set the initial variance
beta_0 = 2000;
[Psi, Lambda, ~] = CreatePsi( n_rfs, beta_0 );
...
for iter=1:n_iter
    ...
    if iter>1
        ...
        for i=1:min(iter,35)
            ...
            j = s_Return(end+1-i,2);
			% calculate the exploration with respect to the current parameters
            temp_explore = (ones(length(rt),1)*(Psi'*(param(:,j)-current_param))')';
			% repeat the Q values
            temp_Q = (Q(:,j)*ones(1,n_rfs))';
            var_nom = var_nom + sum(temp_explore.^2.*temp_Q,2);
            var_dnom = var_dnom + sum(temp_Q,2);
        end
		% apply and an upper and a lower limit to the exploration
        variance(:,iter+1) = var_nom./(var_dnom+1.e-20);
        rate(:,iter+1) = variance(:,iter+1) ./ variance(:,1);
        variance(:,iter+1) = max(variance(:,iter+1), 0.1.*variance(:,1));
        variance(:,iter+1) = min(variance(:,iter+1), 300.*variance(:,1));
    end
...
save('cor_bk_learned.txt','-ascii','Return')
